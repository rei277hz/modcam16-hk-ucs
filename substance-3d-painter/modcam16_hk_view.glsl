...
void shade(V2F inputs){
    ...
    if(!validJhk(base)||!validJhk(emit)){outputDiagnostic(DIAG_DOMAIN);return;}float hdr203_diffuse_white_scale=2.03;vec3 bxyz=adaptD65(decodeJhk(base)*hdr203_diffuse_white_scale,target), exyz=adaptD65(decodeJhk(emit)*hdr203_diffuse_white_scale,target);vec3 scene=inverseAces(bxyz)+inverseAces(exyz);if(!finite3(scene)){outputDiagnostic(DIAG_SCENE);return;}outputUnlit(scene);
}
